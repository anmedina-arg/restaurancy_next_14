...
// const sleep = (ms: number) => new Promise((resolve) => setTimeout(resolve, ms));s

const fetchingDataFromGoogleSheets = async () => {
  const [, ...data] = await fetch(
    "https://docs.google.com/spreadsheets/d/e/2PACX-1vTZq0ny5nDXMUCMnGzu5lRiDQhmTB2BuBzuZtOgyAFzjNotR7X8V_FktuH9yPJTszib2pB55F8c2mDH/pub?output=csv",
  )
    .then((res) => res.text())
...
const parseRowToRestaurant = (row: string): Restaurant => {
...
const api = {
  listGoogleSheets: async (): Promise<Restaurant[]> => {
    // me va a traer los datos de google sheets
    const data = await fetchingDataFromGoogleSheets();

    const restaurants: Restaurant[] = data.map((row) => parseRowToRestaurant(row));
